...
for _, row in df.iterrows():
    alert_icons = ""
    if "予算90%超" in row["alerts"]: alert_icons += " 🔴"
    if "CPA超過"  in row["alerts"]: alert_icons += " 🟡"
    if "Meta未計測" in row["alerts"]: alert_icons += " ⚠️"

    with st.container(border=True):
        c1,c2,c3,c4,c5,c6,c7 = st.columns([2,2,2,1,2,2,1])
        c1.markdown(f"**{row['client']}**{alert_icons}")
